...
class GaleShapleyMatcher:
    ...
    def run(self) -> GaleShapleyOutput:
        ...
        rounds = 0
        # Guard: if no student has eligible preferences (all blocked by hard constraints)
        # return everyone as unmatched immediately instead of crashing on max([])
        pref_lengths = [len(v) for v in self.student_prefs.values() if v]
        if not pref_lengths:
            return GaleShapleyOutput(
                matched=[],
                unmatched=[UnmatchedStudent(sid, "no_eligible_jd") for sid in all_students],
                rounds=0,
            )
        max_rounds = len(all_students) * (max(pref_lengths) + 1)
